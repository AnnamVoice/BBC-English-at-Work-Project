...
//  SearchBarInNav
//
//  Created by Find & Follow on 10/5/18.
...
class MainViewController: UIViewController, UISearchBarDelegate, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    func setupDataWhenOnOffLine(){
        ...
        if CheckInternet.Connection() {
            ...
            dataListObjectSBA = dataListObjectSBNO.sorted(by: {
                $0.title! < $1.title!
            })
            dataListObjectSRAO = dataListObjectSBNO.sorted(by: {
                $0.title! > $1.title!
            })
        } else {
